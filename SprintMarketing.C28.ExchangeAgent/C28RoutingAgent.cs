...
namespace Rhesus.Agents.SprintMarketing
{
    public class SprintAgentFactory : RoutingAgentFactory
    {
        public override RoutingAgent CreateAgent(SmtpServer server)
        {

            var myAgent = new SprintRoutingAgent
            {
                IgnoreDomains = server.AcceptedDomains.Select(x => x.NameSpecification).ToList()
            };
    ...
    public class SprintRoutingAgent : RoutingAgent
    {
        ...
        public SprintRoutingAgent()
        {
            try
            {
                String configPath = Path.Combine(Environment.GetEnvironmentVariable("C28AgentInstallDir"), "config.json");
                SprintRoutingAgent.currentConfig = C28AgentConfig.createConfig(configPath);
                C28Logger.Setup(SprintRoutingAgent.currentConfig);
                ...
                C28CacheManager cache_manager = new C28CacheManager(currentConfig.getAsString(C28ConfigValues.FETCH_CACHE_FILE), currentConfig.getAsInteger(C28ConfigValues.FETCH_INTERVAL_MIN));
                ...
                try
                {
                    if (!cache_manager.isValid() || currentConfig.getAsBoolean(C28ConfigValues.FETCH_EAGER))
                    {
                        ...
                        cache_manager.updateCache(this.exchangeDomains);
                    }
                    else {
                        C28Logger.Info(C28Logger.C28LoggerType.AGENT, "Cache is still valid. Cached data will be used.");
                        this.exchangeDomains = cache_manager.retrieveFromCache();
                    }
                }
                catch (Exception e)
                {
